...
interface IComponent<T> {
    ...
    addListener(element:HTMLButtonElement|HTMLInputElement,type:string,callback:TAction):void;
...
export abstract class Component<T> implements IComponent<T> {
    protected _callbacks: Map<HTMLButtonElement|HTMLInputElement,TeventListener[]>;
    protected constructor(protected readonly _container: HTMLElement, protected readonly _event:IEvents) {
    ...
    public addListener(element:HTMLButtonElement|HTMLInputElement,type:string,callback:TAction) {
    ...
    public clearListeners(element?:HTMLButtonElement|HTMLInputElement) {
        if (!element) {
            this._callbacks.forEach((listeners,element) => {
                listeners.forEach(listener=>element.removeEventListener(listener.type,listener.callback))
            });
        }
        else {
            this._callbacks.get(element).forEach(listener=>element.removeEventListener(listener.type,listener.callback));
        }
    }    
